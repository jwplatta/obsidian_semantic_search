...
(async () => {
  ...
  const countVssTextChunks = db.prepare('SELECT count(*) FROM vss_text_chunks;').pluck().get();

  console.log("countVssTextChunks: ", countVssTextChunks);
  ...
  const insertEmbeddingsIntoVSS = `
    ...
    FROM text_chunks
    WHERE rowid NOT IN (SELECT rowid FROM vss_text_chunks)
  `;
  try {
